...
yl <- unique(yo[, .(id = lsoa21cd, name = lsoa21nm)])[unique(lkp[, .(LSOA, parent = MSOA)]), on = c(id = 'LSOA')]
ym <- fread(
...
ym <- ym[yl, on = c(MSOA = 'parent')][, LSOAn := paste(MSOAn, substring(name, nchar(name)))]
...
lca <- rbindlist(list(
    ...
    ym[, .('LSOA', id, LSOAn, MSOA, substr(id, 1, 1))],
    unique(ym[, .('MSOA', id = MSOA, MSOAn)])[unique(lkp[, .(MSOA, LTLA, substr(MSOA, 1, 1))]), on = c(id = 'MSOA')],
